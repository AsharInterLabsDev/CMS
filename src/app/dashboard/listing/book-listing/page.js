...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        await API.DeleteListingsBooks(id);
        // Remove the deleted item from the state
        setImageMap(prevImageMap => prevImageMap.filter(item => item.id !== id));
      } catch (error) {
        console.error("Error deleting listing book:", error);
      }
    };
    fetchData();
  }, [id]);
                  ...
                  {imageMap.map((gallery, i) => {
                    return (
                      <tr key={i}>
                        ...
                        <td class="whitespace-nowrap flex gap-4 px-4 py-2">
                          <a
                            href={`/dashboard/galleries/updategallery?update=${gallery.id}`}
                            class="my-10 min-w-[100px] text-lg text-center inline-block rounded bg-indigo-600 px-3 py-2 font-medium text-white hover:bg-transparent hover:text-black border border-indigo-600"
                          >
                            Update
                          </a>
                          ...
                          <a
                            ...
                            class="my-10 min-w-[100px] text-lg text-center inline-block rounded bg-red-600 px-3 py-2 font-medium text-white hover:bg-transparent hover:text-black border border-red-600"
                          >
                            Delete
                          </a>
                        </td>
